...
export default function Home() {
  ...
  const handleDrop = async (event, day, timeIndex, yearLevel) => {
    ...
    if (assignment.type === "time") {
      ...
      updatedAssignment = {
        ...
        subjectId: assignment.subjectId,
        teacherId: assignment.teacherId,
        classId: classes.find((c) => c.yearLevel === yearLevel)?.id || "",
        day,
        timeSlot: timeSlots[timeIndex],
        duration: 180,
        type: "time",
    ...
    for (let i = startSlot; i < startSlot + span; i++) {
      dayAssignments.forEach(({ start, span: assSpan, assignment: ass }) => {
        if (start <= i && start + assSpan > i) {
          if (ass.teacherId === updatedAssignment.teacherId) conflicts.push("Teacher already assigned at this time.");
          if (ass.classId === updatedAssignment.classId) conflicts.push("Class already scheduled at this time.");
          ...
          if (assRoomId && newRoomId === assRoomId) conflicts.push("Room already occupied.");
        }
      });
    }
  ...
  const handleSaveEdit = async (assignmentId, updatedData) => {
    ...
    for (let i = startSlot; i < startSlot + span; i++) {
      dayAssignments.forEach(({ start, span: assSpan, assignment: ass }) => {
        if (start <= i && start + assSpan > i) {
          if (ass.teacherId === newData.teacherId) conflicts.push("Teacher already assigned at this time.");
          if (ass.classId === newData.classId) conflicts.push("Class already scheduled at this time.");
          const assRoomId = getAssignmentRoom(ass) !== "N/A" ? roomAssignments.find(
            (ra) =>
              ra.scheduleFileId === ass.scheduleFileId &&
              ra.subjectId === ass.subjectId &&
          ...
          if (assRoomId && assRoomId === newRoomId) conflicts.push("Room already occupied.");
        }
      });
    }
  ...
  return (
    <div className={`p-4 flex ${isFullScreen ? 'flex-col h-screen' : 'flex-col lg:flex-row gap-4'}`} style={{ zIndex: 10 }}>
      <div className={`bg-white rounded-lg ${isFullScreen ? 'flex-1' : 'mb-6 flex-1'}`}>
        ...
        {grids.map(({ program, grid, levels }, progIndex) => (
          <div key={progIndex}>
            ...
            <div
              ...
              style={{
                maxHeight: isFullScreen ? 'calc(100vh - 100px)' : 'calc(100vh - 200px)',
                zIndex: 10,
              }}
            >
              <div
                      ...
                      timeSlots.forEach((timeSlot, timeIndex) => {
                        ...
                        row.push(
                          <td
                            ...
                            className={`border border-darknavy p-1 text-xs bg-darknavy text-white ${bgColor}`}
                                ...
                                <div className="flex items-center justify-start h-full px-1">
                                  <div className="relative group">
                                    ...
                                    <div
                                      ...
                                      style={{ zIndex: 20 }} // Lower z-index for tooltips
                                    >
